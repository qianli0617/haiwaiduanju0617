...
class Auth extends \fast\Auth
{
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        foreach ($ruleList as $k => &$v) {
            ...
            // TODO  分站点主站不显示
            if(strpos($v['name'], 'dramas') !== false && !in_array(2, $this->getGroupIds())){
                unset($ruleList[$k]);
                continue;
                // if($v['name'] != 'dramas/setting' && $v['name'] != 'dramas'){
                //     if($v['name'] != 'dramas/config' && $v['name'] != 'dramas'){
                //     }
                // }
            }
            if(strpos($v['name'], 'dramas') !== false && in_array(2, $this->getGroupIds())){
                if($v['name'] == 'dramas/draw_prompts_config'){
                    unset($ruleList[$k]);
                    continue;
                }
            }
