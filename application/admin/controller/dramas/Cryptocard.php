...
use addons\dramas\library\Export;
...
class Cryptocard extends Backend
{
    ...
    /**
     * Cryptocard模型对象
     * @var \app\admin\model\dramas\Cryptocard
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\dramas\Cryptocard;
        ...
        if ($this->request->isAjax())
        {
            ...
            $total = $this->model
                ...
                ->where(function ($query) use ($searchWhere) {
                    if ($searchWhere) {
                        $table_name = (new \app\admin\model\dramas\Cryptocard())->getQuery()->getTable();
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                if ($success > 0) {
                    $this->success(__('Successfully generated %s card passwords in total', $success));
                } else {
    ...
    {
        if($data['type'] == 'vip'){
            return \app\admin\model\dramas\Vip::where('id', $data['item_id'])->value('title');
        }elseif($data['type'] == 'reseller'){
            return \app\admin\model\dramas\Reseller::where('id', $data['item_id'])->value('name');
        }elseif($data['type'] == 'usable'){
            return \app\admin\model\dramas\Usable::where('id', $data['item_id'])->value('title');
        }
        return null;
    }
    ...
    private function getUserCryptocard($data){
        if($data['status'] == 1){
            $item = Db::name('dramas_user_cryptocard')
                ->alias('uc')
    ...
    public function selectSearch($type='all')
    {
        if ($this->request->isAjax()) {
            $list = [];
            if($type == 'vip'){
                $list1 = \app\admin\model\dramas\Vip::where(['status'=>'1','site_id'=>$this->auth->id])
            ...
            if($type == 'reseller'){
                $list2 = \app\admin\model\dramas\Reseller::where(['status'=>'normal','site_id'=>$this->auth->id])
            ...
            if($type == 'usable'){
                $list3 = \app\admin\model\dramas\Usable::where(['status'=>'1','site_id'=>$this->auth->id])
                    ->order('weigh desc, id asc')
                    ->field('id, title')
                    ->select();
                if($list3){
    ...
    public function export(){
        ...
        for ($i = 0; $i < $total_page; $i++) {
            ...
            $export->exportExcel('List-' . date('Y-m-d H:i:s'), $expCellName, $newList, $spreadsheet, $sheet, [
                'page' => $page,
