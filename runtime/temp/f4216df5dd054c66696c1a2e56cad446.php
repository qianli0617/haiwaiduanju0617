...
<div id="videoDetail" v-cloak style="height: calc(100vh - 63px);" @click.stop="openCategory(0)">
    <div class="preview-body">
        <el-steps :active="stepActive" simple>
            <el-step :title="__('Basic')"></el-step>
            <el-step :title="__('Performer/episode')"></el-step>
            <el-step :title="__('Dramas details')"></el-step>
        </el-steps>
        <div class="good-detail-body">
            <el-form :model="videoDetail" :rules="rules" ref="videoDetail" label-width="110px" class="demo-videoDetail">
                <div v-if="stepActive==1">
                    <el-form-item :label="__('Lang_id')" prop="lang_id">
                        <div class="display-flex">
                            <div class="flex-1">
                                <el-select v-model="videoDetail.lang_id" :placeholder="__('Select')" size="small" @change="langChange">
                    ...
                    <el-form-item :label="__('Subtitle')" prop="subtitle">
                        <el-input type="input" v-model="videoDetail.subtitle" size="small"></el-input>
                    </el-form-item>

                    <el-form-item :label="__('Flags')">
                        <div class="display-flex">
                            <div class="flex-1">
                                <el-select v-model="timeData.flags_arr" :placeholder="__('Select')" multiple size="small" @change="flagsChange">
                    ...
                    <el-form-item :label="__('Status')" prop="status">
                        <el-radio-group v-model="videoDetail.status">
                            <el-radio label="up">{{__('Status up')}}</el-radio>
                            <el-radio label="down">{{__('Status down')}}</el-radio>
                        </el-radio-group>
                    </el-form-item>
                    <el-form-item :label="__('Weigh')" prop="weigh">
                        <div class="display-flex">
                            <el-input type="number" v-model="videoDetail.weigh" style="width:300px;" size="small">
                            </el-input>
                        </div>
                    </el-form-item>
                    <el-form-item :label="__('Image')" prop="image">
                        <div class="display-flex">
                            ...
                            <div class="msg-tip">{{__('Suggested size ratio: 3:4')}}</div>
                        </div>
                    </el-form-item>
                    <el-form-item :label="__('Category_id')" v-if="videoDetail.lang_id">
                        <div class="display-flex">
                            <el-popover placement="bottom-start" width="600" v-model="popperVisible">
                                <div @click.stop="openCategory(1)">
                                    ...
                                    <div style="width: 100%;text-align: center;"
                                         v-if="categoryOptions && categoryOptions.length==0">{{__('No category, please create it')}}</div>
                                </div>
                                <template #reference>
                          ...
                          border-radius: 4px;background: #f9f9f9;" v-for="(tag,key) in tempCategory.label" :key="tag">
                            ...
                            <div style="cursor: pointer;color: #7438D5;margin-left: 30px;flex-shrink: 0;"
                                 @click="createCategory">{{__('Add category')}}
                            </div>
                        </div>
                    </el-form-item>
                    <el-form-item :label="__('Area_id')" v-if="videoDetail.lang_id">
                        <div class="display-flex">
                            <div class="flex-1">
                                <el-select v-model="videoDetail.area_id" :placeholder="__('Select')" size="small">
                    ...
                    <el-form-item :label="__('Year_id')" v-if="videoDetail.lang_id">
                        <div class="display-flex">
                            <div class="flex-1">
                                <el-select v-model="videoDetail.year_id" :placeholder="__('Select')" size="small">
                    ...
                    <el-form-item :label="__('Price')" prop="price">
                        <div class="display-flex">
                            ...
                            <div class="msg-tip">{{__('The price of the entire short dramas without VIP')}}</div>
                        </div>
                    </el-form-item>
                    <el-form-item :label="__('Vprice')" prop="vprice">
                        <div class="display-flex">
                            <el-input v-enter-number type="number" v-model="videoDetail.vprice"
                                      style="width:300px" size="small">
                                <template slot="append">{{__('Point')}}</template>
                            </el-input>
                            <div class="msg-tip">{{__('The price of the entire short dramas with VIP')}}</div>
                        </div>
                    </el-form-item>
                    <el-form-item :label="__('Episodes')" prop="episodes">
                        <el-input type="number" v-model="videoDetail.episodes" size="small"></el-input>
                    </el-form-item>
                    <el-form-item :label="__('Score')" prop="score">
                ...
                <div v-if="stepActive==2">
                                    ...
                                    <div  class="display-flex">
                                        ...
                                        <div class="display-flex" v-if="item.tags_arr.length<3">
                                            <el-popover placement="top" width="160" v-model="allEditPopoverItems[i].tag">
                                                <el-input v-model="allEditItems" size="small">
                                                </el-input>
                                                <div style="text-align: right; margin: 0">
                                                    <el-button size="mini" type="text"
                                                               @click="allEditItem('tag','cancel', i)">{{__('Cancel')}}</el-button>
                                                    <el-button type="primary" size="mini"
                                                               @click="allEditItem('tag','define', i)">{{__('Ok')}}</el-button>
                                                </div>
                                ...
                                <td>
                                    <div class="display-flex">
                                        <el-radio-group v-model="item.type">
                                            <el-radio label="director">{{__('Type director')}}</el-radio>
                                            <el-radio label="performer">{{__('Type performer')}}</el-radio>
                                        </el-radio-group>
                                        <el-input class="table-input"  :placeholder="__('Play')"
                    ...
                    <div class="table-box" style="max-height: 400px;">
                        <table class="table">
                            <thead>
                            <tr>
                                <th>{{__('Episode image')}}</th>
                                <th>{{__('Episode name')}}</th>
                                <th>
                                    <div class="display-flex">
                                        <span>{{__('Episode price')}}({{__('Point')}})</span>
                                        <el-popover placement="top" width="160" v-model="allEditPopover.price">
                                            <el-input v-enter-number v-model="allEditDatas" size="small">
                                            </el-input>
                                            <div style="text-align: right; margin: 0">
                                                <el-button size="mini" type="text"
                                                           @click="allEditData('price','cancel')">{{__('Cancel')}}</el-button>
                                                <el-button type="primary" size="mini"
                                                           @click="allEditData('price','define')">{{__('Ok')}}</el-button>
                                            </div>
                                            <div slot="reference">
                                                <img class="all-edit-img"
                                                     src="/assets/addons/dramas/img/batch.png">
                                            </div>
                                        </el-popover>
                                    </div>
                                </th>
                                <th>
                                    <div class="display-flex">
                                        <span>{{__('Episode vprice')}}({{__('Point')}})</span>
                                        <el-popover placement="top" width="160" v-model="allEditPopover.vprice">
                                            <el-input v-enter-number v-model="allEditDatas" size="small">
                                            </el-input>
                                            <div style="text-align: right; margin: 0">
                                                <el-button size="mini" type="text"
                                                           @click="allEditData('vprice','cancel')">{{__('Cancel')}}</el-button>
                                                <el-button type="primary" size="mini"
                                                           @click="allEditData('vprice','define')">{{__('Ok')}}</el-button>
                                            </div>
                                            <div slot="reference">
                                                <img class="all-edit-img"
                                                     src="/assets/addons/dramas/img/batch.png">
                                ...
                                <th>{{__('Episode video')}}</th>
                                <th>{{__('Episode duration')}}</th>
                                <th>{{__('Weigh')}}</th>
                                <th>{{__('Status')}}</th>
                                <th>
                                    <div class="display-flex">
                                        <span>{{__('Fake_likes')}}</span>
                                        <el-popover placement="top" width="160" v-model="allEditPopover.fake_likes">
                                            <el-input v-enter-number v-model="allEditDatas" size="small">
                                            </el-input>
                                            <div style="text-align: right; margin: 0">
                                                <el-button size="mini" type="text"
                                                           @click="allEditData('fake_likes','cancel')">{{__('Cancel')}}</el-button>
                                                <el-button type="primary" size="mini"
                                                           @click="allEditData('fake_likes','define')">{{__('Ok')}}</el-button>
                                            </div>
                                            <div slot="reference">
                                                <img class="all-edit-img"
                                                     src="/assets/addons/dramas/img/batch.png">
                                ...
                                <th>
                                    <div class="display-flex">
                                        <span>{{__('Fake_views')}}</span>
                                        <el-popover placement="top" width="160" v-model="allEditPopover.fake_views">
                                            <el-input v-enter-number v-model="allEditDatas" size="small">
                                            </el-input>
                                            <div style="text-align: right; margin: 0">
                                                <el-button size="mini" type="text"
                                                           @click="allEditData('fake_views','cancel')">{{__('Cancel')}}</el-button>
                                                <el-button type="primary" size="mini"
                                                           @click="allEditData('fake_views','define')">{{__('Ok')}}</el-button>
                                            </div>
                                            <div slot="reference">
                                                <img class="all-edit-img"
                                                     src="/assets/addons/dramas/img/batch.png">
                                ...
                                <th>
                                    <div class="display-flex">
                                        <span>{{__('Fake_favorites')}}</span>
                                        <el-popover placement="top" width="160" v-model="allEditPopover.fake_favorites">
                                            <el-input v-enter-number v-model="allEditDatas" size="small">
                                            </el-input>
                                            <div style="text-align: right; margin: 0">
                                                <el-button size="mini" type="text"
                                                           @click="allEditData('fake_favorites','cancel')">{{__('Cancel')}}</el-button>
                                                <el-button type="primary" size="mini"
                                                           @click="allEditData('fake_favorites','define')">{{__('Ok')}}</el-button>
                                            </div>
                                            <div slot="reference">
                                                <img class="all-edit-img"
                                                     src="/assets/addons/dramas/img/batch.png">
                                ...
                                <th>
                                    <div class="display-flex">
                                        <span>{{__('Fake_shares')}}</span>
                                        <el-popover placement="top" width="160" v-model="allEditPopover.fake_shares">
                                            <el-input v-enter-number v-model="allEditDatas" size="small">
                                            </el-input>
                                            <div style="text-align: right; margin: 0">
                                                <el-button size="mini" type="text"
                                                           @click="allEditData('fake_shares','cancel')">{{__('Cancel')}}</el-button>
                                                <el-button type="primary" size="mini"
                                                           @click="allEditData('fake_shares','define')">{{__('Ok')}}</el-button>
                                            </div>
                                            <div slot="reference">
                                                <img class="all-edit-img"
                                                     src="/assets/addons/dramas/img/batch.png">
                            ...
                            <tr v-for="(item, i) in videoEpisodes" :key="i">
                                ...
                                <td>
                                    ...
                                    <!--</div>-->
                                        ...
                                        <div class="display-flex">
                                            <div class="display-flex">
                                                <el-input style="width: 200px;" class="table-input" v-model="item.video" @change="getVideoDuration(i)">
                                                </el-input>
                                            </div>
                                            <div class="display-flex table-upload-img" @click="addImg('episodes_video',i,false)">
                                                <i class="el-icon-plus" style="font-size:18px;"></i>
                                            </div>
                                        </div>
                                ...
                                <td>
                                    <el-input class="table-input" v-model="item.duration" type="number" min="0">
                                    </el-input>
                                </td>
                                <td>
                                ...
                                <td>
                                            <span class="sku-status th-center" @click="editStatus(i)">
                                                {{item.status=='normal'?__('Status normal'):__('Status hidden')}}
                                            </span>
                                </td>
                                <td>
                ...
                <div v-if="stepActive==3">
                        ...
                        <div  class="display-flex">
                            ...
                            <div class="display-flex" v-if="timeData.tags_arr.length<5">
                                <el-popover placement="top" width="160" v-model="allEditPopoverTags.tag">
                                    <el-input v-model="allEditTags" size="small">
                                    </el-input>
                                    <div style="text-align: right; margin: 0">
                                        <el-button size="mini" type="text"
                                                   @click="allEditTag('tag','cancel')">{{__('Cancel')}}</el-button>
                                        <el-button type="primary" size="mini"
                                                   @click="allEditTag('tag','define')">{{__('Ok')}}</el-button>
                                    </div>
                                    <div slot="reference" class="add-tags">
                                        <i class="el-icon-plus"></i>
                                    </div>
                    ...
                    <el-form-item :label="__('Description')" prop="description">
                        <el-input type="textarea" v-model="videoDetail.description" rows="3" size="small"></el-input>
                    </el-form-item>
                </div>
    ...
    <span slot="footer" class="dialog-footer">
        <div class="back-btn" v-if="stepActive>1" @click="gonextback">{{__('Previous step')}}</div>
        <div class="btn-common sub-btn" v-if="stepActive<3" @click="gotoback('videoDetail')">{{__('Next step')}}</div>
        <div class="btn-common sub-btn" v-if="stepActive==3" @click="submitForm('videoDetail')">{{__('Ok')}}</div>
    </span>
</div>
                            </div>
                        </div>
